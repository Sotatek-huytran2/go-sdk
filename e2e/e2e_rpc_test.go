...
func TestGetBalance(t *testing.T) {
	...
	acc, err := ctypes.AccAddressFromBech32(testAddress)
	assert.NoError(t, err)
	balance, err := c.GetBalance(acc, "AXC")
	assert.NoError(t, err)
...
func TestSendToken(t *testing.T) {
	...
	assert.NoError(t, err)
	res, err := c.SendToken([]msg.Transfer{{testacc, []ctypes.Coin{{"AXC", 100000}}}}, rpc.Sync, transaction.WithMemo("123"))
	assert.NoError(t, err)
